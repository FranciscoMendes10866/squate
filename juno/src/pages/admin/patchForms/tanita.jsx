...
const PatchTanita = ({ location }) => {
  const { id: tanitaId } = queryString.parse(location.search);
  const userId = useStore(state => state.selectedClient)
  const classes = useStyles();
  console.log({ userId, tanitaId })
  return (
    <>
      <Breadcrumbs aria-label="breadcrumb" className={classes.breadcrumb}>
        ...
        <Link
          color="inherit"
          to={`/dashboard/clients/patch/tanita?id=${tanitaId}`}
          className={classes.crumText}
        >
          Atualizar Avaliação Tanita
        </Link>
      </Breadcrumbs>
